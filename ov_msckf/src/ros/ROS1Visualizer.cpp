...
void ROS1Visualizer::visualize_odometry(double timestamp) {
 ...
 if (!_app->get_propagator()->fast_state_propagate(state, timestamp, state_plus, cov_plus))
  ...
  if (pub_odomworld.getNumSubscribers() != 0) {
    ...
    odomBinB0.twist.twist.linear.x = v_BinW(0);   // vel in world frame
    odomBinB0.twist.twist.linear.y = v_BinW(1);   // vel in world frame
    odomBinB0.twist.twist.linear.z = v_BinW(2);   // vel in world frame
